...
const buildGetManyTemplate = exports.buildGetManyTemplate = (mapping) => {
    return SqlTemplate.parse([
        "select ", getColumns(mapping),
        " from unnest(#{ids}::", mapping.id.type, "[]) with ordinality as t(id, index)",
        " join ", database.getFqn(mapping.table, mapping.schema),
        " on ", mapping.id.column, " = t.id",
        " order by t.index"
    ].join(""));
};
