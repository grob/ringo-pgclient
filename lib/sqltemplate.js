...
const setParams = (statement, statementMetaData, mapping, params) => {
    if (mapping.length > 0) {
        if (!params) {
            throw new Error("Missing query parameter object")
        } else if (!Object.prototype.isPrototypeOf(params)) {
    ...
    mapping.forEach((name, index) => {
        ...
        const parameterType = statementMetaData.getParameterTypeName(position);
        const dataType = dataTypes.DATA_TYPES[parameterType];
        if (!dataType) {
            throw new Error("No data type found for '" + parameterType + "'");
        }
...
SqlTemplate.prototype.execute = function(client, params) {
    ...
    try {
        if (log.isDebugEnabled()) {
            log.debug("'{}' (params: {})", this.templateDesc.sql,
        ...
        log.error("Statement '{}' with params {} failed with: {}",
                this.templateDesc.sql, params ? JSON.stringify(params) : "(no params)", e);
        throw e;
    } finally {
