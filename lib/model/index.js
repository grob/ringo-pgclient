...
const mergeModelData = (mapping, props, data) => {
    return Object.keys(mapping.properties).reduce((params, key) => {
        ...
        return params;
    }, {"id": (data[mapping.id.column] === null || data[mapping.id.column] === undefined) ? props.id : data[mapping.id.column]});
};
...
const defineModel = (client, type, mapping) => {
    ...
    Object.defineProperties(ctor, {
        ...
        /**
         * Returns the number of persisted model instances matching the given `where` clause
         * @param {String} clause The query string clause (`where ...`)
         * @param {Object} params An object containing the parameters referenced in the query clause
         * @name Model.prototype.query
         * @return {Number} The number of persisted model instances
         */
        "count": {
            "value": (clause, params) => {
                return SqlTemplate.newInstance(utils.buildCountTemplate(mapping, clause))
                        .execute(client, params)[0].count;
            }
        },
        /**
