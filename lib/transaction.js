...
const TRANSACTION = module.singleton("transaction-thread-local", () => {
    ...
    this.commit = function() {
        ...
        if (client.cache) {
            [inserted, updated].forEach((map) => {
                Object.keys(map).forEach((key) => client.cache.put(key, map[key]._data));
            });
            Object.keys(deleted).forEach((key) => client.cache.remove(key));
        }
        client.onCommit(this.keys);
        reset();
    };
