...
const getBit = (resultSet, index) => resultSet.getString(index);
const setBit = (statement, index, value) => statement.setObject(index, value, java.sql.Types.OTHER);
...
const setTime = (statement, index, value) => statement.setTime(index, new PGTime(value));
...
const setTsVector = (statement, index, value) => {
...
const setTsQuery = (statement, index, value) => {
    const obj = new PGobject();
    obj.setType(TSQUERY);
    obj.setValue(value);
    statement.setObject(index, obj, java.sql.Types.OTHER);
...
DATA_TYPES[BIT] = new DataType(BIT, getBit, setBit);
DATA_TYPES[BIT_VARYING] = new DataType(BIT_VARYING, getBit, setBit);
...
DATA_TYPES[TSQUERY] = new DataType(TSQUERY, getString, setTsQuery);
DATA_TYPES[TSVECTOR] = new DataType(TSVECTOR, getString, setTsVector);
// TODO: UUID
// TODO: XML
